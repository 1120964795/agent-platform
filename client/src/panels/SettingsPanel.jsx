    ...
    <div className="p-6 space-y-5">
      ...
      {tab === 'models' && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold">Qwen</h2>
          ...
          <label className="block space-y-2 text-xs text-[color:var(--text-muted)]">Primary model<input value={form.qwenPrimaryModel} onChange={(event) => patch({ qwenPrimaryModel: event.target.value })} className="w-full rounded-md border border-[color:var(--border)] bg-[color:var(--bg-primary)] px-3 py-2 text-sm text-[color:var(--text-primary)] outline-none focus:border-[color:var(--accent)]" /></label>
          <label className="block space-y-2 text-xs text-[color:var(--text-muted)]">Coding model<input value={form.qwenCodingModel} onChange={(event) => patch({ qwenCodingModel: event.target.value })} className="w-full rounded-md border border-[color:var(--border)] bg-[color:var(--bg-primary)] px-3 py-2 text-sm text-[color:var(--text-primary)] outline-none focus:border-[color:var(--accent)]" /></label>

          <div className="border-t border-[color:var(--border)] pt-4 space-y-3">
            <h2 className="text-lg font-semibold">DeepSeek fallback</h2>
            <p className="text-xs text-[color:var(--text-muted)]">Optional plain-chat fallback only. DeepSeek is never used for task planning or action intent.</p>
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.fallbackProvider === 'deepseek'} onChange={(event) => patch({ fallbackProvider: event.target.checked ? 'deepseek' : '' })} /> Enable plain-chat fallback</label>
      ...
      {tab === 'runtimes' && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold">External runtimes</h2>
          <label className="block space-y-2 text-xs text-[color:var(--text-muted)]">Open Interpreter endpoint<input value={form.openInterpreterEndpoint} onChange={(event) => patch({ openInterpreterEndpoint: event.target.value })} placeholder="http://127.0.0.1:8756" className="w-full rounded-md border border-[color:var(--border)] bg-[color:var(--bg-primary)] px-3 py-2 text-sm text-[color:var(--text-primary)] outline-none focus:border-[color:var(--accent)]" /></label>
          <label className="block space-y-2 text-xs text-[color:var(--text-muted)]">Open Interpreter command<input value={form.openInterpreterCommand} onChange={(event) => patch({ openInterpreterCommand: event.target.value })} placeholder="external sidecar command" className="w-full rounded-md border border-[color:var(--border)] bg-[color:var(--bg-primary)] px-3 py-2 text-sm text-[color:var(--text-primary)] outline-none focus:border-[color:var(--accent)]" /></label>
          <label className="block space-y-2 text-xs text-[color:var(--text-muted)]">UI-TARS endpoint<input value={form.uiTarsEndpoint} onChange={(event) => patch({ uiTarsEndpoint: event.target.value })} placeholder="http://127.0.0.1:8765" className="w-full rounded-md border border-[color:var(--border)] bg-[color:var(--bg-primary)] px-3 py-2 text-sm text-[color:var(--text-primary)] outline-none focus:border-[color:var(--accent)]" /></label>
          <label className="block space-y-2 text-xs text-[color:var(--text-muted)]">UI-TARS command<input value={form.uiTarsCommand} onChange={(event) => patch({ uiTarsCommand: event.target.value })} placeholder="external adapter command" className="w-full rounded-md border border-[color:var(--border)] bg-[color:var(--bg-primary)] px-3 py-2 text-sm text-[color:var(--text-primary)] outline-none focus:border-[color:var(--accent)]" /></label>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.uiTarsScreenAuthorized} onChange={(event) => patch({ uiTarsScreenAuthorized: event.target.checked })} /> Screen authorization active</label>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.dryRunEnabled} onChange={(event) => patch({ dryRunEnabled: event.target.checked })} /> Enable dry-run mode</label>
        </div>
      )}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-2">
            <button type="button" onClick={() => patch({ permissionMode: 'default' })} className={`flex items-start gap-3 rounded-md border p-3 text-left ${!isFull ? 'border-[color:var(--accent)] bg-[color:var(--accent)]/5' : 'border-[color:var(--border)] hover:bg-[color:var(--bg-tertiary)]'}`}>
              ...
              <div><div className="text-sm font-medium">Chat</div><div className="text-xs text-[color:var(--text-muted)]">Normal conversation. Execution tasks still use brokered proposals.</div></div>
            </button>
            <button type="button" onClick={() => patch({ permissionMode: 'full' })} className={`flex items-start gap-3 rounded-md border p-3 text-left ${isFull ? 'border-[color:var(--success)] bg-[color:var(--success)]/5' : 'border-[color:var(--border)] hover:bg-[color:var(--bg-tertiary)]'}`}>
              ...
              <div><div className="text-sm font-medium">Compatibility tools</div><div className="text-xs text-[color:var(--text-muted)]">Legacy local tools remain hidden from Execute mode.</div></div>
            </button>
          </div>
          ...
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.session_confirm_cache_enabled} onChange={(event) => patch({ session_confirm_cache_enabled: event.target.checked })} /> Remember approved compatibility shell commands for this session</label>
        </div>
      )}
